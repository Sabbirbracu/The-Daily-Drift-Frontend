  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white px-4 sm:px-6 py-6">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Profile Overview - Responsive Layout */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 mb-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
            <img
              ...
              alt="Profile"
              className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-yellow-500 object-cover"
            />
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <p className="text-xl sm:text-2xl font-semibold">{profile.fullName}</p>
                {profile.role === "admin" && (
              ...
              {/* On mobile, show buttons under name */}
              <div className="flex sm:hidden mt-3 gap-2">
                <button
                  onClick={() => navigate(`/dashboard-${user.role}/edit-profile`)}
                  className="bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-md text-sm font-medium"
                >
                  Edit Profile
                </button>
                <button
                  onClick={() =>
                    navigate(`/dashboard-user/profile-details/${profile.displayName}`)
                  }
