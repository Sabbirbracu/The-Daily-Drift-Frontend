...
const UserDashboard = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 min-h-screen bg-gray-950 text-white">
      <div className="relative mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center sm:text-left">Welcome Back 👋</h1>
        <Button
          onClick={() => refetch()}
          variant="outline"
          className="flex items-center gap-2 border border-gray-300 text-white hover:bg-white hover:text-black"
        >
          <RotateCcw size={18} />
          Refresh
        </Button>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            <StatCard title="Your Posts" value={totalPosts} icon="file-text" bgColor="bg-blue-100" textColor="text-blue-800" />
            <StatCard title="Total Likes" value={totalLikes} icon="thumbs-up" bgColor="bg-green-100" textColor="text-green-800" />
            <StatCard title="Categories Written" value={categoryStats.length} icon="list" bgColor="bg-purple-100" textColor="text-purple-800" />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10 mb-10">
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">📊 Posts by Category</h3>
              <PieChartComponent data={categoryStats.length > 0 ? categoryStats : [{ name: "No Data", value: 1 }]} />
            </div>
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">📈 Post Growth Over Time</h3>
              <LineChartComponent data={growthData} xKey="month" yKey="posts" />
            </div>
          </div>

          {topLikedPost && topLikedPost.likes?.length > 0 ? (
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md mb-10 text-gray-900">
              <h2 className="text-xl font-semibold mb-2">🔥 Top Liked Post</h2>
              <p className="text-lg font-medium text-blue-600">{topLikedPost.title}</p>
              <p className="text-sm text-gray-500 mt-1">Likes: {topLikedPost.likes.length}</p>
            </div>
          ) : (
            <div className="bg-white p-6 rounded-xl shadow-md mb-10 border border-dashed text-center text-gray-700">
              <h2 className="text-xl font-semibold mb-2">😔 No Top Liked Post Yet</h2>
              <p className="text-sm mb-1">It looks like none of your posts have received likes yet.</p>
              <p className="text-sm mb-4">Create awesome content and share it with your friends to get more engagement!</p>
              <Link
                to={`/dashboard-${user.role.toLowerCase()}/create-post`}
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md text-gray-900">
